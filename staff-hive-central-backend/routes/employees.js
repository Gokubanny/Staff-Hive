...
router.get('/', auth, async (req, res) => {
  try {
    const { search, page = 1, limit = 10, department, status } = req.query;
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    }).populate('companyId', 'name').populate('userId', 'name email');
...
router.post('/', auth, employeeValidation, async (req, res) => {
  try {
    ...
    await newEmployee.populate('userId', 'name email');
...
// Get employee by user ID (for when employees sign in)
